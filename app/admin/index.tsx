    ...
    try {
      if (isEditing) {
        const { error } = await supabase
          .from('access_tokens')
          .update({ username: newUsername, token: newToken })
          .eq('id', isEditing);
        ...
        const { error } = await supabase
          .from('access_tokens')
          .insert([{ username: newUsername, token: newToken, is_admin: false }]);
    ...
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      <View style={[styles.header, { borderBottomColor: theme.borderStrong }]}>
        <View style={styles.headerLeft}>
          <View style={[styles.miniLogo, { backgroundColor: theme.surfaceElevated, borderColor: theme.borderStrong }]}>
            <Svg width="20" height="20" viewBox="0 0 50 49">
              <Path d={STAR_PATH} fill={theme.text} />
            </Svg>
          </View>
          <Text style={[styles.headerTitle, { color: theme.text }]}>ADMIN_CONSOLE_v1.0</Text>
        </View>
        <TouchableOpacity onPress={logout} style={[styles.logoutBtn, { borderColor: theme.borderStrong, backgroundColor: theme.error }]}>
